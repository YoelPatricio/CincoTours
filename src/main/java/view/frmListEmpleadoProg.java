...
public class frmListEmpleadoProg extends javax.swing.JDialog {

    DefaultTableModel dtm;
    String accion = "";
    int contador = 0;
    public boolean piloto=false;

    public frmListEmpleadoProg(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void tableDatosMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tableDatosMouseClicked
        if(evt.getClickCount()==2){
            ...
            String tipo = String.valueOf(tableDatos.getValueAt(selection, 5));
            String estado = String.valueOf(tableDatos.getValueAt(selection, 6));
            if(estado.equals("OCUPADO")){
                JOptionPane.showMessageDialog(null, "El Empleado seleccionado se encuentra ocupado !");
                return;
            }
            if(tipo.equalsIgnoreCase("Piloto")){
                frmProgramacion.txtIdPiloto.setText(String.valueOf(tableDatos.getValueAt(selection, 0)));
                frmProgramacion.txtDNIPiloto.setText(String.valueOf(tableDatos.getValueAt(selection, 1)));
                frmProgramacion.txtNombPiloto.setText(String.valueOf(tableDatos.getValueAt(selection, 2)));
                this.dispose();
            }else{
                frmProgramacion.txtIdCoPiloto.setText(String.valueOf(tableDatos.getValueAt(selection, 0)));
                frmProgramacion.txtDNICoPiloto.setText(String.valueOf(tableDatos.getValueAt(selection, 1)));
                frmProgramacion.txtNombCoPiloto.setText(String.valueOf(tableDatos.getValueAt(selection, 2)));
                this.dispose();
            }
            //frmProgramacion.tx
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(frmListEmpleadoProg.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                frmListEmpleadoProg dialog = new frmListEmpleadoProg(new javax.swing.JFrame(), true);
    ...
    private void cargarDatos() {
        ...
        String valCoPi = frmProgramacion.txtValCoPi.getText();
        List<Object> datos;
        if(valCoPi.equals("PILOTO")){
            datos = empDAO.buscarPiloto(txtBuscar.getText());
        }else{
            datos = empDAO.buscarCoPiloto(txtBuscar.getText());
        }
        ...
        Object[][] data = matriz;
        String[] cabecera = {"Codigo", "N° DNI", "Nombres y Apellidos", "N° Celular", "Dirección","Rol","Estado"};
